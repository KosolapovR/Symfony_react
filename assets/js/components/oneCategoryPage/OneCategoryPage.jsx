...
function OneCategoryPage({match, location, history, ...props}) {
    const classes = useStyles();

    useEffect(() => {
        props.getCategory(match.params.categoryId);
    }, []);

    let doctors;
    if (props.category.users !== undefined) {
        doctors = Object.values(props.category.users);
    }

    console.log(doctors);
    props.category.users ?
        doctors = doctors.map((c, i) => <Box key={i}><ListItem alignItems="flex-start">
    ...
    if (props.isFetching) {
        ...
        return (
            <>
                <Typography component='div'>Специализация: {props.category.name}</Typography>
                <List subheader='Доктора'>{doctors}</List>
            </>
        );
    }
}
...
function mapStateToProps(state) {
    return {
        category: state.categories.oneCategory,
        isFetching: state.categories.isFetching
    }
}

const WithRouterOneCategoryPage = withRouter(OneCategoryPage);

export default connect(mapStateToProps, {getCategory})(WithRouterOneCategoryPage);